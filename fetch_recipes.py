...
def connect_elasticsearch():
    ...
    # pings the server and returns True if connected
    if _es.ping():
        print('Yay connect')
    else:
...
def store_record(elastic_object, index_name, record):
    is_stored = True
    try:
        outcome = elastic_object.index(index=index_name,
                                       doc_type='salaads',
                                       body=record)
        print(outcome)
    except Exception as ex:
...
def parse(u):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # calories
            calories_section = soup.select('.calories-count')
            if calories_section:
            ...
            if ingredients_section:
                for ingredient in ingredients_section:
                    ...
                    if 'Add all ingredients to list' not in ingredient_text and \
                            ingredient_text != '':
